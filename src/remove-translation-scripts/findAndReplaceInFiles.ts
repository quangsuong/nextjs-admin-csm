...
const getUrlPattern3 = /getLocalizedUrl\((.*), locale as Locale\)/g
...
const removeParamsFromFunctionPattern2 =
  /\(\{\s*params\s*\}\s*:\s*\{\s*params\s*:\s*\{\s*lang:\s*Locale\s*\}\s*\}\s*\)/g
...
async function replacePatternInFile(filePath: string) {
  ...
  if (
    ...
    removeParamsFromFunctionPattern2.test(data)
  ) {
    ...
    const newData = data
      ...
      .replace(removeParamsFromFunctionPattern2, '()')
